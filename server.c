...
/*char *get_content(char *path){
    ...
    if(ptr){
        if(strcmp(ptr, ".css") == 0) return "text/css";
        if(strcmp(ptr, ".csv") == 0) return "text/csv";
        if(strcmp(ptr, ".gif") == 0) return "image/gif";
        if(strcmp(ptr, ".htm") == 0) return "text/html";
        if(strcmp(ptr, ".html") == 0) return "text/html";
        if(strcmp(ptr, ".ico") == 0) return "image/x-icon";
        if(strcmp(ptr, ".jpeg") == 0) return "image/jpeg";
        if(strcmp(ptr, ".jpg") == 0) return "image/jpeg";
        if(strcmp(ptr, ".js") == 0) return "application/javascript";
        if(strcmp(ptr, ".json") == 0) return "application/json";
        if(strcmp(ptr, ".png") == 0) return "image/png";
        if(strcmp(ptr, ".pdf") == 0) return "application/pdf";
        if(strcmp(ptr, ".svg") == 0) return "image/svg+xml";
        if(strcmp(ptr, ".txt") == 0) return "text/plain";
    }
}*/
...
void img_partition(char *tmp, char *style){
    char *ptr = strstr(tmp, "/mnt/c/Users/paul/Desktop");
    while(*ptr != ' '){
        *style = *ptr;
        ptr++;
        style++;
...
void conn_handler(int fd){
    ...
    if(strncmp(tmp[0], "GET", 3) == 0){
        if(strstr(tmp[0], ".jpg") > 0){
            img_partition(tmp[0], style);
            printf("%s\n", style);
            char img_header[] =
                "HTTP/1.1 200 OK\r\n"
    ...
    if(strncmp(tmp[0], "POST", 4) == 0){
        ...
        char *bound_ptr;
        length = atoi(num);
        char **data_storage = malloc(length * sizeof(char *));
        int idx = 0;
